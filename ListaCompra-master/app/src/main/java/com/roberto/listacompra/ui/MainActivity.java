...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
         ...
         */
        listaCompraViewModel = ViewModelProviders.of(this).get(ListaCompraViewModel.class);

        //Con esto averiguamos si hay datos (>0)
        if(listaCompraViewModel.getProductos().getValue().size()>0){
            try{
                getAssets().open("productos.json");

                listaCompraViewModel.insertarProductos(listaProductos/*ArrayList*/)//Crearlo
